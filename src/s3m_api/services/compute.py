...
class ComputeService:
    ...
    def __init__(self, cluster_name : str, auth_context : S3MAuthContext):
        self._client = auth_context
        self._cluster_name = cluster_name
        self._service_url = f'{auth_context.base_url}/slurm/v0.0.42/{cluster_name}'
...
'''{{
   "job": {{
        ...
        print(f"Submission as string: {submit_request_str}") 

        # submit_request = submit_request_str.encode()
        # TYLER: encode not necessary, actually it creates an I/O error when it reaches slurm... 
        print('hear ye') 
        """   
        job_dict = {
            "job": {
            ...
            "environment": env_vars if env_vars else ["HELLO=world"],  # Safe default,
            "nodes": str(node_count),
        ...
        response = requests.post(url=submit_url, json=job_dict,
