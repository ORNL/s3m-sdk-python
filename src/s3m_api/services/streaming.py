...
from ..auth import S3MAuthContext
...
class StreamingService:
    """
    def __init__(self, service_name : str, api_client : S3MAuthContext):
    ...
    """
    def __init__(self, auth_context : S3MAuthContext):
        self._auth = auth_context
        self._base_url = f'{auth_context.base_url}/olcf/v1alpha/streaming'
        # self._service_name = service_name
        # self._service_url = f'{api_client.base_url}/olcf/v1alpha/streaming/{service_name}'
        # self._cluster_name = "unknown"
        # self._cluster_provisioned = False
    ...
    def list_backends(self) -> List[str]:
        """Returns a list of available streaming backend names."""
        list_url = f'{self._auth.base_url}/olcf/v1alpha/streaming/list_backends'

        try:
            response = requests.get(
                url=list_url,
                headers={"Authorization": f'{self._auth.api_token}'}
            )
            response.raise_for_status()

            service_list = response.json()
            return service_list.get("backends", [])

        except requests.RequestException as e:
            raise RuntimeError(f"Failed to fetch streaming backends from {list_url}: {e}")
    """ 
    ...
    def start_cluster(self,
        ...
        provision_url = f'{self._base_url}/{backend_name}/provision_cluster'
        provision_template = \
'''{{
    "kind": "{kind}",
    "name": "{cluster}",
...
'''{{
    ...
    "resourceSettings": {{
        ...
        response = requests.post(url=provision_url, data=provision_request,
                                 ...
                                 headers={"Authorization": f'{self._auth.api_token}', "Content-Type": "application/json"})
        ...
        if response:
            ...
            return True, json.loads(provision_details)
        else:
    ...
    def stop_cluster(self, cluster_name : str) -> Tuple[bool, str]:
        ...
        if response:
            ...
            return False, error
    """
    def stop_cluster(self, backend_name: str, cluster_name: str) -> Tuple[bool, str]:
        cluster_url = f'{self._base_url}/{backend_name}/cluster/{cluster_name}'
        ...
        try:
            ...
            shutdown_details = json.dumps(response.json(), indent=4)
            shutdown_info = f'INFO: {backend_name} cluster shutdown\n{shutdown_details}'
            return True, shutdown_info

        except requests.RequestException as e:
            error = f'DELETE {cluster_url} failed - {e}'
            print(f'ERROR: {error}')
            return False, error
    """
    def list_clusters(self) -> Tuple[bool, str]:
        ...
        if response:
            ...
            return False, error
    """
    # TODO: TYLER MAKE THESE THE SAME TYPE. 
    def list_clusters(self, backend_name: str):
        list_url = f'{self._base_url}/{backend_name}/list_clusters'
        ...
        try:
            ...
            cluster_list = response.json()
            clusters = json.dumps(cluster_list.get("clusters", []), indent=4)
            return json.loads(clusters)

        except requests.RequestException as e:
            error_msg = f'GET from {list_url} failed - {e}'
